...
import {DataResponse, LoanTypeItem} from '@/app/types'
...
const CreateEditLoanType: FC<Props> = ({handleClose, data, handleUpdated}) => {
  const {rows, settings} = LOAN_TYPE_CONFIG
  ...
  async function handleCreateOrUpdate() {
    ...
    try {
      const {data: dataRes} = data?.id
        ? await request.put<DataResponse<LoanTypeItem>>(endpoint + '/' + data?.id, newPayload) // edit
        : await request.post<DataResponse<LoanTypeItem>>(endpoint || '', newPayload) // create
      ...
      const message = `Loan Type "${dataRes?.data?.type_name}" successfully ${
  ...
  return (
    ...
    >
      ...
      <div className='modal-header p-30px'>
        <h2 className='m-0'>{data?.id ? 'Edit' : 'New'} Loan Type</h2>
...
export default CreateEditLoanType

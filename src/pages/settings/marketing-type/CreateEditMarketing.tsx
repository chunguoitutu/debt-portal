...
import {DataResponse, MarketingTypeItem} from '@/app/types'
import {CheckboxRounded} from '@/components/checkbox'
import clsx from 'clsx'
import {MARKETING_TABLE_CONFIG} from './config'

type Props = {
  data?: MarketingTypeItem
  handleClose: () => void
  handleUpdated: () => void
}

const CreateEditMarketingType: FC<Props> = ({handleClose, data, handleUpdated}) => {
  const {rows, settings} = MARKETING_TABLE_CONFIG
  ...
  async function handleCreateOrUpdate() {
    ...
    try {
      const {data: dataRes} = data?.id
        ? await request.put<DataResponse<MarketingTypeItem>>(endpoint + '/' + data?.id, payload) // edit
        : await request.post<DataResponse<MarketingTypeItem>>(endpoint || '', payload) // create
      ...
      const message = `Marketing Type "${dataRes?.data?.marketing_type_name}" successfully ${
  ...
  return (
    ...
    >
      ...
      <div className='modal-header p-30px'>
        <h2 className='m-0'>{data?.id ? 'Edit' : 'New'} Marketing Type</h2>
...
export default CreateEditMarketingType

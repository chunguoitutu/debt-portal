  ...
  return (
    <div className='d-flex flex-column gap-16px'>
      {CONFIG_PROFILE.map((profile, i) => {
        return (
          <div key={i} className='d-flex  gap-16px align-items-center'>
            ...
            <p className='col-9 fs-14 fw-semibold text-gray-900 p-0 m-0'>
              {data?.profile?.[profile.key] || false ? (
                profile.key === 'gender' ? (
                  capitalizeFirstLetter(data?.profile?.[profile.key] || '')
                ) : (
                  `${profile.startTest}${data?.profile?.[profile.key]}`
                )
              ) : (
                <span className=' text-capitalize none-company-detail m-0 p-0 h-100 fs-14 fw-semibold text-gray-800 font-italic'>
                  None
                </span>
              )}
            </p>

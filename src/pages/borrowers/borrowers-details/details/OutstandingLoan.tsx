...
import {CONFIG_REPAYMENT_SCHEDULE_CUSTOMER} from '../config'

const OutstandingLoan: FC<LoanDetailsProps> = ({loanInfo}) => {
  ...
  const configTable = useMemo(() => {
    ...
    const newRows = CONFIG_REPAYMENT_SCHEDULE_CUSTOMER.rows.filter(
      (el) => !keyIgnored.includes(el.key)
    )
    return {
      ...CONFIG_REPAYMENT_SCHEDULE_CUSTOMER,
      rows: newRows,
    }
  ...
  function handleInitialConfigColumn() {
    let config = CONFIG_REPAYMENT_SCHEDULE_CUSTOMER.rows
      .filter((el) => !el.isHide)
  ...
  function handleResetConfigColumn() {
    const config = CONFIG_REPAYMENT_SCHEDULE_CUSTOMER.rows
      .filter((el) => !el.isHide)
      .reduce((acc, el) => ({...acc, [el.key]: el.defaultShow === false ? false : true}), {})
      ...
      <div className='d-flex align-items-center justify-content-between gap-16px py-16px border-bottom border-gray-200 position-relative'>
        ...
        <div className={clsx(['position-relative ', showConfigColumn && 'text-gray-900'])}>
          ...
          {showConfigColumn && (
            <div className='config-column-grid card' ref={selectRef}>
              ...
              {/* Body */}
              <div className='grid-2-column d-flex flex-column  gap-16px mh-300px overflow-y-auto px-30px fw-semibold py-24px'>
                {CONFIG_REPAYMENT_SCHEDULE_CUSTOMER.rows
                  .filter((el) => el.key !== 'actions')
                  .map((el, i) => {
        ...
        {/* {showFilterPopup && (
          <FilterPopup
            ...
            rows={CONFIG_REPAYMENT_SCHEDULE_CUSTOMER.rows}
          />
        )}
        ...
        <div className='d-flex justify-content pt-14px m-0'>
          ...
          <div className='d-flex justify-content-start align-items-center p-0 m-0 flex-wrap'>
            {Object.keys(filterHasValue).map((key, i) => {
              const currentConfig = CONFIG_REPAYMENT_SCHEDULE_CUSTOMER.rows.find((row) => row.key === key)
